...
//		getTitleLayout().getChildAt(count - 1).setVisibility(View.GONE);
		...
		} else { // def{note.disable_popup_animation.def{lang}}
	...
	private void interceptWeiboActionListener(String weiboName) {
		...
		// def{note.backup_walistener.def{lang}}
		backListener = weibo.getWeiboActionListener();
		// def{note.set_intercept_wal.def{lang}}
		weibo.setWeiboActionListener(this);
	}

	public void onError(AbstractWeibo weibo, int action, Throwable t) {
		if (action == AbstractWeibo.ACTION_AUTHORIZING) { // def{note.error_when_auth.def{lang}}
			weibo.setWeiboActionListener(backListener);
			if (backListener != null) {
				backListener.onError(weibo, action, t);
			}
		}
		else { // def{note.error_when_auth.def{lang}}
			weibo.setWeiboActionListener(backListener);
			if (backListener != null) {
	...
	public void onComplete(AbstractWeibo weibo, int action,
		...
		if (action == AbstractWeibo.ACTION_FOLLOWING_USER) { // def{note.pretend_do_nothing_after_auth.def{lang}}
			weibo.setWeiboActionListener(backListener);
			if (backListener != null) {
				backListener.onComplete(weibo, AbstractWeibo.ACTION_AUTHORIZING, null);
			}
		}
		else if (ctvFollow.isChecked()) { // def{note.follow_after_auth.def{lang}}
		...
		else { // def{note.ret_if_auth_and_follow_not_checked.def{lang}}
			weibo.setWeiboActionListener(backListener);
			if (backListener != null) {
				backListener.onComplete(weibo, AbstractWeibo.ACTION_AUTHORIZING, null);
			}
	...
	public void onCancel(AbstractWeibo weibo, int action) {
		weibo.setWeiboActionListener(backListener);
		if (action == AbstractWeibo.ACTION_AUTHORIZING) { // def{note.cancel_before_auth.def{lang}}
			if (backListener != null) {
				backListener.onCancel(weibo, action);
			}
		}
		else { // def{note.pretend_do_nothing_after_auth.def{lang}}
			if (backListener != null) {
				backListener.onComplete(weibo, AbstractWeibo.ACTION_AUTHORIZING, null);
			}
